...
for(i in genes){
  ...
  values <- grep(paste(patternA, pattern0, pattern1, pattern2, pattern3, pattern4, pattern5, pattern6, sep = "|"), OutFile, value = TRUE)
  
  ### extract ln values so that I can calculate 2*deltaln
  lnL_M1 <- str_extract(values[3], "(-)\\d+\\.\\d+")
  lnL_M2 <- str_extract(values[6], "(-)\\d+\\.\\d+")
  lnL_M7 <- str_extract(values[10], "(-)\\d+\\.\\d+")
  lnL_M8 <- str_extract(values[14], "(-)\\d+\\.\\d+")
  ### calculate 2*deltaln
  ...
  M1_par <- paste(values[4], values[5])
  M2_par <- paste(values[7], values[8])
  M7_par <- c(values[11])
  M8_par <- paste(values[15], values[16])
