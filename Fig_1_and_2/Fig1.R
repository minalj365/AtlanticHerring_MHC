...
######### Fig 1B ###########
dir = "C:/Users/minal03/OneDrive - Texas A&M University/U-Drive/MHC/Sequence_analysis/Fig1_tree/"
in_file <- readDNAStringSet(paste0(dir, "Ref_alpha.fa"))
align <- msa(in_file, "Muscle", order = "aligned")

labels <- data.frame(seq = rownames(align),
                     category = c(rep("DB", 2), rep("DA", 5)))

dist <- dist.dna(as.DNAbin(align), model = "JC69", pairwise.deletion = T) # ape
tree <- bionj(dist)

Fig1B <- ggtree(tree, size=0.7) %<+% labels + geom_tiplab(hjust = -0.05, fontface = "italic", size = 5, aes(color = category)) +
  scale_color_manual(values=c(DA = "#c1272d",DB= "#008176")) +
...
labels <- data.frame(seq = rownames(align),
                     category = c(rep("DB", 4), rep("DA", 6)))

dist <- dist.dna(as.DNAbin(align), model = "JC69", pairwise.deletion = T) # ape
tree <- bionj(dist)

Fig1C <- ggtree(tree, size=0.7) %<+% labels + geom_tiplab(hjust = -0.05, fontface = "italic", size = 5, aes(color = category)) +
  scale_color_manual(values=c(DA = "#c1272d",DB= "#008176")) +
  theme(legend.position='none') +
  geom_treescale() + geom_rootedge() +
  labs(tag = "(C)") +
...
library(patchwork)
dir <- "C:/Users/minal03/OneDrive - Texas A&M University/U-Drive/MHC/Figures/dotplot_haplotypes/"


ggsave(paste0("C:/Users/minal03/OneDrive - Texas A&M University/U-Drive/MHC/Figures/Fig1_new.png"), Fig1, width = 20, height = 23, dpi = 300, units = "cm")
ggsave(paste0("C:/Users/minal03/OneDrive - Texas A&M University/U-Drive/MHC/Figures/Fig1_new.pdf"), Fig1, width = 20, height = 23, dpi = 300, units = "cm")
